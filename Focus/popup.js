const el = {
    ...
    customShieldDomain: document.getElementById('customShieldDomain'),
    addCustomShieldBtn: document.getElementById('addCustomShieldBtn'),
    shieldCurrentBtn: document.getElementById('shieldCurrentBtn'),
    shieldHotkeyInput: document.getElementById('shieldHotkeyInput'),
    clearHotkeyBtn: document.getElementById('clearHotkeyBtn'),
    shieldModeBlur: document.getElementById('shieldModeBlur'),
    shieldModeBlack: document.getElementById('shieldModeBlack')
};
let state = {
    duration: 25,
    timeLeft: 25 * 60,
...
async function init() {
    ...
    if (!state.allShieldSites || state.allShieldSites.length === 0) {
        state.allShieldSites = [
            { domain: 'discord.com', name: 'Discord' },
        ...
        if (state.shieldList) {
            state.shieldList.forEach(d => {
                if (!state.allShieldSites.some(s => s.domain === d)) {
                    state.allShieldSites.push({ domain: d, name: d });
                }
            });
        }
        chrome.storage.local.set({ allShieldSites: state.allShieldSites });
    }
...
function switchTab(tabId, save = true) {
    const btn = Array.from(el.tabBtns).find(b => b.dataset.tab === tabId);
...
function renderSiteList() {
    ...
    sitesToShow.forEach(site => {
        ...
        const displayDomain = site.domain.length > 25 ? site.domain.substring(0, 22) + '...' : site.domain;
        const displayName = site.name.length > 20 ? site.name.substring(0, 17) + '...' : site.name;
        item.innerHTML = `
            <img src="${getFavicon(site.domain)}" alt="" class="site-icon">
            <span class="site-name" title="${site.name}">${displayName}</span>
            <span class="site-domain" title="${site.domain}">${displayDomain}</span>
            <div class="toggle-switch ${isActive ? 'active' : ''}" data-domain="${site.domain}" role="button" aria-label="Toggle block">
        ...
        toggle.onclick = (e) => {
            ...
            item.classList.toggle('blocked', isNowActive);
            toggleSite(site.domain);
        };
...
function toggleSite(domain) {
    const currentList = state.whitelistMode ? state.whitelist : state.blocklist;
    const isAdding = !currentList.includes(domain);
    if (!isAdding) {
...
function updateStats() {
    ...
    const logsKey = `siteVisitLogs_${dateStr}`;
    chrome.storage.local.get(['totalFocusTime', 'sessionsCompleted', 'currentStreak', 'siteTimeData', statsKey, logsKey], (data) => {
        ...
        el.siteTimeList.innerHTML = sites.map(([domain, seconds]) => {
            const pct = Math.round((seconds / maxTime) * 100);
            const domainLogs = visitLogs[domain] || [];
            return `
                <div class="site-time-container" data-domain="${domain}">
                    ...
                    <div class="site-logs" style="display: none;">
                        ${domainLogs.length > 0 ?
                    domainLogs.slice().reverse().map(ts => `
                                <div class="log-entry">
                                    <span class="log-bullet"></span>
                                    Entered at ${new Date(ts).toLocaleTimeString([], { hour: 'numeric', minute: '2-digit', hour12: true })}
                                </div>
                            `).join('') :
                    '<div class="log-entry empty">No entry logs for today</div>'
                }
                    </div>
                </div>
            `;
        }).join('');
        el.siteTimeList.querySelectorAll('.site-time-container').forEach(container => {
            container.querySelector('.site-time-item').onclick = () => {
                const logs = container.querySelector('.site-logs');
                const chevron = container.querySelector('.chevron-icon');
                const isHidden = logs.style.display === 'none';
                el.siteTimeList.querySelectorAll('.site-logs').forEach(l => l.style.display = 'none');
                el.siteTimeList.querySelectorAll('.chevron-icon').forEach(c => c.style.transform = '');
                if (isHidden) {
...
function loadSettings() {
    ...
    el.shortsToggle.checked = state.shortsEnabled;
...
el.shortsToggle.onchange = () => {
    state.shortsEnabled = el.shortsToggle.checked;
    chrome.storage.local.set({ shortsEnabled: state.shortsEnabled });
    playSound('toggle');
};
...
el.quoteBtn.onclick = async () => {
    ...
    const res = await callBG('GET_QUOTE');
    el.apiResult.textContent = res.success ? `"${res.data.text}" - ${res.data.author}` : "Error";
};
function userUpdateShieldUI() {
    el.shieldToggleBtn.classList.toggle('active', state.shieldEnabled);
    el.shieldStatusText.textContent = state.shieldEnabled ? 'Shield ON' : 'Shield OFF';
    renderShieldList();
}
function renderShieldList() {
    el.shieldList.innerHTML = '';
    if (!state.allShieldSites || state.allShieldSites.length === 0) {
        el.shieldList.innerHTML = '<p class="empty-state">No shielded sites added</p>';
        return;
    }
    state.allShieldSites.forEach(site => {
        const isActive = state.shieldList.includes(site.domain);
        const item = document.createElement('div');
        item.className = 'site-item';
        const displayName = site.name.length > 30 ? site.name.substring(0, 27) + '...' : site.name;
        item.innerHTML = `
            <img src="${getFavicon(site.domain)}" alt="" class="site-icon">
            <span class="site-name" title="${site.name}">${displayName}</span>
            <label class="switch" style="flex-shrink:0;">
                <input type="checkbox" ${isActive ? 'checked' : ''} data-domain="${site.domain}">
                <span class="slider round"></span>
            </label>
            <button class="small-btn remove-site-btn" style="background:transparent; color:var(--text-dim); padding:4px; flex-shrink:0;" title="Remove">
                <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
            </button>
        `;
        const checkbox = item.querySelector('input[type="checkbox"]');
        checkbox.onchange = (e) => {
            toggleShieldSite(site.domain, e.target.checked);
        };
        const removeBtn = item.querySelector('.remove-site-btn');
        removeBtn.onclick = (e) => {
            e.preventDefault();
            e.stopPropagation();
            removeShieldSite(site.domain);
        };
        el.shieldList.appendChild(item);
    });
}
function toggleShieldSite(domain, enabled) {
    if (enabled) {
        if (!state.shieldList.includes(domain)) {
            state.shieldList.push(domain);
        }
    } else {
        state.shieldList = state.shieldList.filter(d => d !== domain);
    }
    skipNextRender = true;
    chrome.storage.local.set({ shieldList: state.shieldList }, () => {
        playSound('toggle');
        setTimeout(() => {
            skipNextRender = false;
            renderShieldList();
...
function removeShieldSite(domain) {
    state.allShieldSites = state.allShieldSites.filter(s => s.domain !== domain);
    state.shieldList = state.shieldList.filter(d => d !== domain);
    chrome.storage.local.set({ allShieldSites: state.allShieldSites, shieldList: state.shieldList });
    renderShieldList();
    playSound('click');
}
function addShieldSite(domain) {
    domain = domain.trim().toLowerCase();
    if (!domain) return;
    domain = domain.replace('https://', '').replace('http://', '').split('/')[0];
    if (!state.allShieldSites) state.allShieldSites = [];
    if (!state.allShieldSites.some(s => s.domain === domain)) {
        state.allShieldSites.push({ domain: domain, name: domain });
    }
    if (!state.shieldList.includes(domain)) {
        state.shieldList.push(domain);
    }
    chrome.storage.local.set({
        allShieldSites: state.allShieldSites,
        shieldList: state.shieldList
    });
    renderShieldList();
    playSound('success');
}
el.shieldToggleBtn.onclick = () => {
    state.shieldEnabled = !state.shieldEnabled;
    chrome.storage.local.set({ shieldEnabled: state.shieldEnabled });
    userUpdateShieldUI();
    playSound('toggle');
};
el.addCustomShieldBtn.onclick = () => {
    addShieldSite(el.customShieldDomain.value);
    el.customShieldDomain.value = '';
};
el.customShieldDomain.onkeypress = (e) => {
    if (e.key === 'Enter') {
        addShieldSite(el.customShieldDomain.value);
        el.customShieldDomain.value = '';
    }
};
el.shieldCurrentBtn.onclick = async () => {
    const tabs = await chrome.tabs.query({ active: true, currentWindow: true });
    if (!tabs[0]?.url) return;
    try {
        let url = new URL(tabs[0].url);
        let domain = url.hostname.replace('www.', '');
        addShieldSite(domain);
    } catch (e) {
    }
};
el.shieldHotkeyInput.addEventListener('keydown', (e) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    const hotkeyStr = combo.join('+');
    state.shieldHotkey = hotkeyStr;
    el.shieldHotkeyInput.value = hotkeyStr;
    chrome.storage.local.set({ shieldHotkey: hotkeyStr });
    playSound('success');
});
el.clearHotkeyBtn.onclick = () => {
    state.shieldHotkey = '';
    el.shieldHotkeyInput.value = '';
    chrome.storage.local.set({ shieldHotkey: '' });
    playSound('click');
};
el.shieldModeBlur.onclick = () => {
    state.shieldOverlayMode = 'blur';
    chrome.storage.local.set({ shieldOverlayMode: 'blur' });
    updateShieldModeUI();
    playSound('toggle');
};
el.shieldModeBlack.onclick = () => {
    state.shieldOverlayMode = 'black';
    chrome.storage.local.set({ shieldOverlayMode: 'black' });
    updateShieldModeUI();
    playSound('toggle');
};
function updateShieldModeUI() {
    el.shieldModeBlur.classList.toggle('active', state.shieldOverlayMode === 'blur');
    el.shieldModeBlack.classList.toggle('active', state.shieldOverlayMode === 'black');
}
function loadShieldSettings() {
    el.shieldHotkeyInput.value = state.shieldHotkey || '';
    updateShieldModeUI();
}
...
chrome.storage.onChanged.addListener((changes) => {
    ...
    for (let key in changes) {
        ...
        if (key === 'shieldList' || key === 'allShieldSites') renderShieldList();
