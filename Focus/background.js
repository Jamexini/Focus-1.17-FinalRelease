...
chrome.runtime.onInstalled.addListener(async (details) => {
    const defaults = {
        ...
        shieldEnabled: false,
        shieldList: ['discord.com', 'web.whatsapp.com', 'instagram.com', 'paypal.com', 'outlook.live.com', 'gmail.com', 'messenger.com', 'telegram.org'],
        shieldHotkey: '',
    ...
    if (details.reason === 'install') {
        ...
        const data = await chrome.storage.local.get(['blocklist', 'whitelist', 'shieldList']);
        ...
        if (!data.shieldList) {
            chrome.storage.local.set({ shieldList: defaults.shieldList, shieldEnabled: false });
        }
    }
...
chrome.tabs.onActivated.addListener(async (activeInfo) => {
    ...
    if (activeTabDomain) logSiteVisit(activeTabDomain);
});
...
async function trackSiteTime(domain, seconds) {
    ...
    const data = await chrome.storage.local.get([statsKey, logsKey]);
    const dayStats = data[statsKey] || {};
    dayStats[domain] = (dayStats[domain] || 0) + seconds;
    await chrome.storage.local.set({ [statsKey]: dayStats });
...
async function logSiteVisit(domain) {
    if (!domain) return;
    const dateStr = getLocalDateString();
    const logsKey = `siteVisitLogs_${dateStr}`;
    const data = await chrome.storage.local.get(logsKey);
    const logs = data[logsKey] || {};
    if (!logs[domain]) logs[domain] = [];
    const now = Date.now();
    logs[domain].push(now);
    if (logs[domain].length > 100) logs[domain].shift();
...
async function cleanupOldStats() {
    ...
    const statKeys = Object.keys(data).filter(k => k.startsWith('stats_'));
    const logKeys = Object.keys(data).filter(k => k.startsWith('siteVisitLogs_'));
    if (statKeys.length > 30) {
        statKeys.sort();
        const toDelete = statKeys.slice(0, statKeys.length - 30);
        await chrome.storage.local.remove(toDelete);
    }
    if (logKeys.length > 30) {
        logKeys.sort();
        const toDelete = logKeys.slice(0, logKeys.length - 30);
        await chrome.storage.local.remove(toDelete);
    }
